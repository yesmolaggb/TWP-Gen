...
class AutoEncoder(nn.Module):
    def __init__(
        self, input_dim1, input_dim2, hidden_dims, agg, sep_decode
    ):
    ...
    def __init__(self, args):
        ...
        if self.agg_method == "concat":
            self.topic_emb = Parameter(
                torch.Tensor(args.n_clusters, 2 * hidden_dims[-1])
            )
        else:
        ...
        if os.path.exists(pretrained_path) and self.args.load_pretrain:
            ...
            for epoch in range(pretrain_epoch):
                ...
                for batch_idx, (x1, x2, _, weight) in enumerate(train_loader):
                    ...
                    optimizer.zero_grad()
                    x_bar1, x_bar2, z = self.model(x1, x2)  # bug
                    loss = (
...
def train(args, emb_dict):
    ...
    # emb_dict["oh_emb"] = np.concatenate(
    #     (emb_dict["oh_emb"], emb_dict["ents_emb"]), axis=1
    # )
    embs2 = F.normalize(torch.tensor(emb_dict["oh_emb"]), dim=-1)
    # tensor2 = torch.tensor(emb_dict["oh_emb"])
    # tensor3 = torch.tensor(emb_dict["ents_emb"])
    # embs2 = torch.cat((tensor2, tensor3), dim=-1)
    ...
    for epoch in range(50):
        if epoch % 5 == 0:
            ...
            if not os.path.exists(
                ...
                os.makedirs(os.path.join(topic_cluster.dataset_path, f"clusters_{args.suffix}"))
